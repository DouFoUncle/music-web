<template>
	<view class="singerInfo-container">
		<view class="vague-wrapper bg-img bg-mask flex align-center" :style="{ 'background-image': 'url(' + singerInfo.headImg + ')' }">
				...
				<view class="description-wrapper">
					...
					<view class="tags flex">
						<view class="cu-capsule round" style="margin-right: 20px;">
							...
							<view class="cu-tag ">{{ singerInfo.musicList.lenght }}</view>
						</view>
					...
					<view class="description" v-if="singerInfo.readme">简介：{{ singerInfo.readme }}</view>
				</view>
			</view>
		</view>
...
export default {
	...
	methods: {
		async getDetail() {
			...
			this.musicList.push(...data.musicList);
		...
		handlePlayMusic(val) {
			...
			this.$store.dispatch('playMusic', {
				...
				title: val.musicName,
				singer: getName(val),
				coverImgUrl: this.singerInfo.headImg,
				id: val.id
			});
		},
		handlePlayAllMusic() {
