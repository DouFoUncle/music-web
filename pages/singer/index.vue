			...
			<block v-for="(item, index) in list" :key="index">
				<view :class="'indexItem-' + item.name" :id="'indexes-' + item.name" :data-index="item.name">
					...
					<view class="cu-list page-background menu-avatar no-padding">
						<view class="cu-item bg-white" v-for="val in item.value" :key="val.id"
							@click="searchDetailSinger(val)">
							<view class="cu-avatar round lg"
								:style="'background-image:url(' + val.headImg + ')'"></view>
							<view class="content">
								<view class="singer-name text-overflow">{{ val.name }}</view>
								<view class="singer-count" v-if="val.sex">{{ val.sex == 0 ? '女' : val.sex == 1 ?'男':'组合' }}</view>
							</view>
						</view>
					</view>
				</view>
