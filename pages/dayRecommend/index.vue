	...
	<view class="day-recommend-container">
		<view class="vague-wrapper flex align-center">
			...
			<view class="absolve-wrapper">
				<cu-custom class="head-title" :isBack="true" bgColor="unset"><block slot="content">全部歌曲</block></cu-custom>
		...
		<view class="recommend-main" :class="{ hasPlayInfo: playInfo.id }">
			<view class="recommend-list">
				...
				<scroll-view scroll-y scroll-with-animation style="height: calc(100% - 55px)">
					...
					>
						<image :src="item.imgUrl" mode="widthFix" class="music-img"></image>
...
export default {
	...
	methods: {
		...
		async getDayRecommendData() {
			const { data } = await this.$api.getMusicList({indexFlag:0});
			this.dayRecommendList = data || [];
			if (data.length > 0) {
				this.bgImg = this.dayRecommendList[0].imgUrl;
			}
		},
