<template>
	<div class="play-container">
		<div class="mask-container" :style="{ 'background-image': 'url(' + playInfo.coverImgUrl + ')' }">
			<!-- <div
        class="album-cover"
      ...
      playInfo.singer
		...
		<view class="img-container" :style="{ transform: 'translate(-50%, -50%) rotate(' + rotate + 'deg)' }"
			v-show="!lyricShow">
			<image :src="playInfo.coverImgUrl" class="authorImg"></image>
		</view>
		<scroll-view scroll-y v-if="lyricShow" @click="lyricShow = false" :scroll-top="scrollTop"
			class="lyric-container" :style="{ top: CustomBar + 35 + 'px' }">
		...
		<view class="bottom-control">
			<view class="progress">
				...
				<slider class="audio-slider" activeColor="rgb(248, 78, 81)" block-size="8"
					:value="playDetailInfo.current_value" :max="playDetailInfo.duration_value" @change="handleChange">
				</slider>
				<view class="audio-number">{{ playDetailInfo.duration }}</view>
			</view>
			<view class="iconList flex">
				<text class="iconfont" :class="isLike? 'icon-like lighIcon': 'icon-unlike'
          " @click="likeMusic()" style="margin-right: 65rpx"></text>
				<text class="iconfont icon-play-left" @click="handleChangePlay(-1)"></text>
				<text class="iconfont" :class="!paused ? 'icon-play' : 'icon-pause'"
...
<script>
	...
	export default {
		...
		onShow() {
			...
			//this.initLyric();
			try {
				this.$audio.syncStateOn("page-index-get-state", ({
					playinfo
				}) => {
					this.playDetailInfo = {
						...playinfo
		...
		methods: {
			...
			async getLikeData() {
				...
				const data = await this.$api.likeData({
					userId
				});
				this.likeList = data.data || [];
				let obj = this.likeList.find(item => {
					return item.musicId == this.playInfo.id
				})
				this.isLike = (obj || '') != ''
			},
			...
			async getLyric(bool) {
				...
				const res = await this.$api.getLyric({
					id
				});
